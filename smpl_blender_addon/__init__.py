...
bl_info = {
    ...
    "description": "SMPL for Blender (Updated for Blender 4.5 compatibility)",
...
class SMPLSetPoseshapes(bpy.types.Operator):
    ...
    def rodrigues_to_mat(self, rotvec):
        ...
        r = (rotvec/theta) if theta > 0. else rotvec
        r = r.reshape(-1)  # Ensure r is a 1D array
        cost = np.cos(theta)
        mat = np.asarray([[0, -r[2], r[1]],
                        [r[2], 0, -r[0]],
                        [-r[1], r[0], 0]])
        r = r.reshape(3, 1)  # Reshape for matrix operations
